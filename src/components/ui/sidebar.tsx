...
function SidebarProvider({
	...
	// Always use defaultOpen as initial state to ensure server/client match for hydration
	const [_open, _setOpen] = React.useState(defaultOpen);
	const open = openProp ?? _open;

	// Handle mounting to prevent hydration mismatch
	// After mount, read cookie and update state if it differs from default
	React.useEffect(() => {
		setMounted(true);

		// Only read cookie if not controlled by props
		if (openProp !== undefined) return;

		// Check cookie on client after mount
